...
int main( int argc, char* args[] ){
	...
	else
	{
		bool done = false;
		//Create window
		window = SDL_CreateWindow( "SDL Tutorial", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, Constant::SCREEN_WIDTH, Constant::SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( window == NULL )
		{
		...
		else
		{
			...
			while(!done)
			{
				...
				enemy = new Enemy(posImg.x, posImg.y);
				enemy->DrawEnemy(screenSurface,bFront);		
                
				SDL_UpdateWindowSurface( window );

				TransPosition = body->GetPosition();

				if(TransPosition.x ==700)
				{
					bTransform = false;
					bFront = false;
				}
				...
				{
					TransPosition.x++;
					body->SetTransform(TransPosition,0);
				}
				else
				{
					TransPosition.x--;
